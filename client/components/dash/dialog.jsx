...
const Dialog = () => {
  ...
  return (
    <div className="relative grow text-gray-600 bg-purple-400 break-all ">
      <div
        id="dialog-window"
        className="absolute flex flex-col w-full h-full p-2 overflow-auto overscroll-auto"
      >
        {dates.map((msgsDate, ind) => {
          return (
            <div className="flex flex-col gap-2" key={ind}>
              <div className="sticky top-0 w-fit self-center bg-gray-300 rounded-full text-center px-4 mb-2 backdrop-blur-md">
                {msgsDate}
              </div>
              {receivedMess.map((msg) => {
